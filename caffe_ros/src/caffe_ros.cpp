...
sensor_msgs::Image::ConstPtr CaffeRos::computeOutputs()
{
    ...
    else if (post_proc_ == PostProc::Segmentation)
    {
      cv::Mat class_map(net_.getOutHeight(), net_.getOutWidth(), CV_8UC1);
      getArgMax(net_.getOutput(), net_.getOutChannels(), class_map);

      out_msg->encoding = "mono8";
      out_msg->width = class_map.cols;
      out_msg->height = class_map.rows;
      out_msg->step = class_map.cols;
      size_t count = out_msg->step * out_msg->height;
      const unsigned char *ptr = class_map.data;
      out_msg->data = std::vector<unsigned char>(ptr, ptr + count);
    }
