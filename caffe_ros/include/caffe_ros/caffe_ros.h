...
private:
    ...
    enum class PostProc
    {
        ...
        Segmentation // Collapse multidimensional softmax into a matrix of class IDs.
    };
...
private:
    ...
    void setPostProcessing(const std::string& postProc)
    {
        ...
        else if (postProc == "Segmentation")
          post_proc_ = PostProc::Segmentation;
        else
        {
            ROS_FATAL("Post processing %s is not supported. Supported: YOLO, Segmentation", postProc.c_str());
            ros::shutdown();
        }
    }
