...
TEST(CaffeRosTests, FCNAlexNetPredictions)
{
    ...
    CaffeRosTestsCallback t;
    auto dnn_sub = nh.subscribe<sensor_msgs::Image>("/fcn_alexnet_dnn/network/output", 1,
                                                    &CaffeRosTestsCallback::dnnCallback, &t);
    const char* camera_topic = "/camera_sidewalk/image_raw";
    auto img_pub = nh.advertise<sensor_msgs::Image>(camera_topic, 1);

    // Test images and expected predictions.
    std::string image{"sidewalk.png"};
    cv::Mat predictions = cv::imread("sidewalk-raw-out.png");
    ...
    EXPECT_TRUE(t.dnn_out_ != nullptr);
    auto dnn_out = *t.dnn_out_;
    // For this size input, the output will be 54 x 11.
    EXPECT_EQ(dnn_out.width,  54);
    EXPECT_EQ(dnn_out.height, 11);
    // Single channel, unsigned char.
    EXPECT_EQ(dnn_out.encoding,  "mono8");

    auto data  = reinterpret_cast<const unsigned char*>(dnn_out.data.data());
    for (size_t y = 0; y < dnn_out.height; y++)
    {
        for (size_t x = 0; x < dnn_out.width; x++)
        {
          unsigned char expected = predictions.at<unsigned char>(y, x);
          unsigned char actual = data[y * dnn_out.height + x];
          
          EXPECT_EQ(expected, actual) << "Values are not equal at (" << x << ", " << y <<")";
        }
    }
}
